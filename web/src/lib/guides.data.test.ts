...
describe("guides data layer (ownership)", () => {
  it("creates a guide owned by the user, with a slug and steps", async () => {
    ...
    const g = await createGuide({ title: "Test", steps: [fixtureStep()] }, u.id);
  ...
  it("lists only the owner's guides with a thumbnail", async () => {
    ...
    await createGuide({ title: "Mine", steps: [fixtureStep()] }, u1.id);
    await createGuide({ title: "Theirs", steps: [fixtureStep()] }, u2.id);

    const mine = await listGuides(u1.id);
    expect(mine).toHaveLength(1);
    expect(mine[0].title).toBe("Mine");
    expect(mine[0].thumbnail).toContain("data:image");
    expect(await listGuides(u2.id)).toHaveLength(1);
  });

  it("getGuide exposes userId so routes can check ownership", async () => {
    const u = await mkUser("o@example.com");
    const g = await createGuide({ title: "Owned", steps: [fixtureStep()] }, u.id);
  ...
  it("updates fields and re-indexes step order", async () => {
    ...
    const g = await createGuide({ title: "Edit", steps: [fixtureStep()] }, u.id);
  ...
  it("resolves a public guide by slug", async () => {
    ...
    const g = await createGuide({ title: "Pub", steps: [fixtureStep()] }, u.id);
  ...
  it("deletes a guide", async () => {
    ...
    const g = await createGuide({ title: "Del", steps: [fixtureStep()] }, u.id);
